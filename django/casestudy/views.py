...
class SecurityView(APIView):
    ...
    def get(self, request, subpath="", format=None):
        ...
        logger.debug(
            {
                "message": f"Security GET view for the API for user {request.user.username}, path: {subpath}"
            }
        )
    ...
    def post(self, request, subpath="", format=None):
        ...
        logger.debug(
            {
                "message": f"Security POST view for the API for user {request.user.username}, path: {subpath}"
            }
        )
    ...
    def __handle_search_ticker(self, request):
        ...
        return Response(
            {"message": f"Ticker {ticker} found", "tickers": ticker_data}, status=200
        )
    ...
    def __handler_add_to_portfolio(self, request):
        ...
        if ticker_already_exists:
            response = Response(
                {"message": f"Ticker {ticker} already in portfolio"}, status=200
            )
        else:
            request.user.securities.create(ticker=ticker)
            response = Response(
                {"message": f"Ticker {ticker} added to portfolio"}, status=200
            )
    ...
    def __handler_remove_from_portfolio(self, request):
        ...
        if not ticker_exists:
            response = Response(
                {"error": f"Ticker {ticker} not in portfolio, cannot remove"},
                status=200,
            )
        else:
            request.user.securities.filter(ticker=ticker).delete()
            response = Response(
                {"message": f"Ticker {ticker} removed from portfolio"}, status=200
            )
